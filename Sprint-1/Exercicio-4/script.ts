...
function changeBio(list: IList[], text: string, item: number): void {
  const index: number = list.findIndex((list) => { return list.id === item});
  if (index != -1) list[index].bio = text;
}

function changeName(list: IList[], text: string, item: number): void {
  const index: number = list.findIndex((list) => { return list.id === item});
...
function showTable(list: IList[]): void{
  ...
  tbody.innerHTML = '';
  list.forEach(function (item: any): void {
