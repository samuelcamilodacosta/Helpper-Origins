...
function changeName(action: string, text: string, i: number): string {
  list[i].name = text;
  return `Changed ${action}`;
}

function changeBio(action: string, text: string, i: number): string {
  list[i].bio = text;
  return `Changed ${action}`;
}
...
function changeBioOrName(list: any[], action: string, text: string, id: number): string {
  let i: number = findItem(id);
  if (i != -1) {
    if (action == 'name') changeName(action, text, i);
    else if (action == 'bio') changeBio(action, text, i);
